        ...
        if self.receipt.rowCount() != 0:
            with open('client.csv', 'w', encoding='utf8') as csvfile:
                ...
                for row in lest:
                    if row:
                        writer.writerow(row)
            self.ordermsg = QMessageBox()
            self.ordermsg.setText('Ваш заказ готовится, пройдите на кассу для оплаты заказа.')
            self.ordermsg.setWindowTitle('Заказ принят')
            self.ordermsg.setWindowIcon(QIcon('pizzaicon.png'))
            self.ordermsg.exec()
...
class CashierForm(QWidget):
    ...
    def acceptOrder(self):
        ...
        print('Заказ в пути!')
        self.ordermsg = QMessageBox()
        self.ordermsg.setText('Заказ оплачен, ожидайте.')
        self.ordermsg.setWindowTitle('Оплата завершена')
        self.ordermsg.setWindowIcon(QIcon('pizzaicon.png'))
        self.ordermsg.exec()
        self.setClientOrder()
    ...
    def declineOrder(self):
        ...
        print('Заказ отменён...')
        self.ordermsg = QMessageBox()
        self.ordermsg.setText('Заказ отменён.')
        self.ordermsg.setWindowTitle('Отмена')
        self.ordermsg.setWindowIcon(QIcon('pizzaicon.png'))
        self.ordermsg.exec()
        self.setClientOrder()
        self.setClientOrder()
